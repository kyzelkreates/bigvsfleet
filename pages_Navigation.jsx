...
import { MapStatusPanel, MapAdvisoryBanner } from './components_ui_MapStatusPanel'
import { DEMO_ROUTE, normaliseExistingRoute, routeToMarkers } from './services_maps_routeProviderManager'
import { getMapProviderStatus, MAP_MODE } from './services_maps_mapProviderManager'
// COMBINED RUN 15-16: Provider adapters + result cards
import { fetchGraphHopperRoute, isGraphHopperConfigured, GH_STATUS } from './services_routing_graphhopperRouteAdapter'
import { fetchRestrictionDataForRoute, getOverpassConfig, OVERPASS_STATUS } from './services_routing_overpassAdapter'
import { OverpassResultCard, GraphHopperResultCard, ProviderStatusStrip } from './components_ui_RouteProviderResultCard'
...
function RoutePlanner({ onRoute, onClear, drivers, vehicles }) {
  ...
  // RUN 15-16: Provider results (advisory)
  const [ghResult,       setGhResult]       = useState(null)
  const [overpassResult, setOverpassResult] = useState(null)
  const [vehicleProfileLocal, setVehicleProfileLocal] = useState('car')
  ...
  const plan = async () => {
    ...
    try {
      ...
      // RUN 15-16: Kick off provider calls (non-blocking, advisory)
      const routeCoords = route?.coordinates || []
      const vProfile    = vehicleProfileLocal || 'car'
      // GraphHopper
      fetchGraphHopperRoute({
        origin:         { lat: origins[0].lat,      lng: origins[0].lng },
        destination:    { lat: destinations[0].lat, lng: destinations[0].lng },
        waypoints:      geocodedStops.map(s => s.geocoded).filter(Boolean),
        vehicleProfile: vProfile,
      }).then(ghRes => setGhResult(ghRes)).catch(() => {})
      // Overpass
      if (routeCoords.length > 1) {
        fetchRestrictionDataForRoute({ coords: routeCoords, vehicleProfile: vProfile }).then(ovpRes => setOverpassResult(ovpRes)).catch(() => {})
      }
    } catch (err) {
    ...
    <>
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[999] w-[440px] pointer-events-auto">
        <div className="bg-[#0d1426]/97 border border-slate-700/50 rounded-xl p-3 backdrop-blur-sm shadow-2xl">
          ...
          {/* RUN 15-16: Vehicle profile selector */}
          <div className="flex items-center gap-1.5 mb-1.5">
            <Icon name="Truck" size={11} className="text-slate-600 flex-shrink-0" />
            <select value={vehicleProfileLocal} onChange={e => setVehicleProfileLocal(e.target.value)}
              className="flex-1 bg-slate-900/70 border border-slate-800/60 rounded text-2xs text-slate-400 px-2 py-1.5 outline-none focus:border-slate-600">
              {[['car','Car / Light Vehicle'],['van','Van / LGV'],['small_truck','Small Truck'],['truck','HGV / Truck'],['bike','Bike']].map(([v,l]) => (
                <option key={v} value={v}>{l}</option>
              ))}
            </select>
            <span className="text-2xs text-slate-700">Advisory</span>
          </div>
          ...
          {result && (
            <div className="mt-2 pt-2 border-t border-slate-800/40">
              ...
              {/* RUN 15-16: Provider Status Strip */}
              <ProviderStatusStrip
                ghResult={ghResult}
                overpassResult={overpassResult}
                ghConfigured={isGraphHopperConfigured()}
                overpassConfigured={!!getOverpassConfig().endpointUrl}
              />

              {/* RUN 15-16: Provider Result Cards */}
              {ghResult && (
                <GraphHopperResultCard result={ghResult} onClose={() => setGhResult(null)} />
              )}
              {overpassResult && (
                <OverpassResultCard result={overpassResult} onClose={() => setOverpassResult(null)} />
              )}
...
export default function Navigation() {
  ...
  // RUN 5: Map Engine Foundation — demo mode + 3D/status state
  ...
  // RUN 5: Demo route fallback — show demo when in demo mode and no real route
  useEffect(() => {
    if (demoMode === 'demo' && mapRoutes.length === 0) {
      // Wire demo route polyline
      const demoCoords = DEMO_ROUTE.coordinates
      setMapRoutes([{
        coordinates: demoCoords,
        color:       '#a78bfa',   // violet — demo colour, distinct from live cyan
        weight:      3,
        opacity:     0.65,
        dashed:      true,        // dashed to signal demo/advisory nature
      }])
      // Wire demo markers
      const demoMarkers = routeToMarkers(DEMO_ROUTE)
      setMapMarkers(prev => {
        const nonRoute = prev.filter(m => !m._routePin)
        return [...nonRoute, ...demoMarkers]
      })
      setDemoRouteActive(true)
    } else if (demoMode !== 'demo' && demoRouteActive) {
      // Clear demo route when switching to live mode
      setMapRoutes([])
      setMapMarkers([])
      setDemoRouteActive(false)
    }
  }, [demoMode, mapRoutes.length, demoRouteActive])
  ...
  return (
    <div className="fixed inset-0 z-30 bg-[#050810] overflow-hidden">
      {/* Exit button + Run 5 controls */}
      <div className="absolute top-4 right-4 z-[1100] flex items-center gap-2">
        {/* 3D toggle — MapLibre readiness (Run 5/6) */}
        <button
          onClick={() => setPrefer3d(p => !p)}
          title={prefer3d ? "Switch to 2D mode" : "3D mode (MapLibre — Run 5/6)"}
          className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium backdrop-blur-sm transition-all shadow-xl border ${
            prefer3d
              ? 'bg-cyan-500/10 border-cyan-500/30 text-cyan-300'
              : 'bg-[#0d1426]/95 border-slate-700/60 text-slate-500 hover:text-slate-300'
          }`}
        >
          <Icon name="Layers" size={13} />
          <span className="hidden sm:inline">{prefer3d ? '3D' : '2D'}</span>
        </button>

        {/* Map status panel toggle */}
        <button
          onClick={() => setShowMapStatus(s => !s)}
          title="Map provider status"
          className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium backdrop-blur-sm transition-all shadow-xl border ${
            showMapStatus
              ? 'bg-slate-700/60 border-slate-600/60 text-white'
              : 'bg-[#0d1426]/95 border-slate-700/60 text-slate-400 hover:text-white'
          }`}
        >
      ...
      {/* RUN 5: Map status panel */}
      {showMapStatus && (
        <div className="absolute top-16 right-4 z-[1100] w-72 pointer-events-auto">
          <MapStatusPanel
            prefer3d={prefer3d}
            onToggle3d={() => setPrefer3d(p => !p)}
            activeRoute={demoRouteActive ? DEMO_ROUTE : null}
            showAdvisory
          />
        </div>
      )}
      ...
      <div className="relative w-full h-full">
        ...
        <Suspense fallback={<MapLoader />}>
          <ApexMap
            ...
            isDemo={demoRouteActive}
            showAdvisory
            activeMode={prefer3d ? 'fallback_2d' : 'osm_2d'}
            mapStatusCompact
          />
        </Suspense>
