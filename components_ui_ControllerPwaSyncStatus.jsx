/**
 * ============================================================
 * Big V's Best Routes™ — Controller PWA Sync Status Strip
 * /src/components/ui/ControllerPwaSyncStatus.jsx
 *
 * RUN 13 — Real-Time Dashboard ↔ Fleet Controller PWA Sync
 *
 * PURPOSE:
 *   Compact sync status strip for Fleet Controller PWA.
 *   Shows: mode, backend status, pending count, last sync, retry.
 *
 * SECURITY:
...
import { useControllerStore } from './core_storage'
import {
...
import { getPendingCount } from './services_sync_offlineQueueManager'
...
function _getMode() {
  try { const r = localStorage.getItem('bigv:mode:demoLive'); return r ? JSON.parse(r)?.demoLiveMode?.mode ?? 'demo' : 'demo' } catch { return 'demo' }
}
...
export default function ControllerPwaSyncStatus({ compact = false }) {
  const syncStatus       = useControllerStore(s => s.syncStatus)
  const [liveSyncStatus, setLiveSyncStatus] = useState(() => getControllerLiveSyncStatus())
  ...
  useEffect(() => {
    const refresh = () => { setLiveSyncStatus(getControllerLiveSyncStatus()); setPending(getPendingCount()); setMode(_getMode()) }
    refresh()
  ...
  const handleRetry = useCallback(async () => {
    ...
    const result = await retryControllerPendingSync()
    setRetrying(false); setRetryResult(result)
  ...
  const isLive          = !isDemo
  const backendMissing  = isLive && !isLiveControllerSyncAvailable()
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between gap-3 px-3 py-2.5 rounded-xl border border-slate-800/40 bg-slate-900/20">
        ...
        <div className="flex items-center gap-2">
          {pending > 0 && <span className="text-2xs text-amber-300 font-semibold">{pending} pending</span>}
          {syncStatus?.lastLocalSaveAt && <span className="text-2xs text-slate-700">{tsAgo(syncStatus.lastLocalSaveAt)}</span>}
        </div>
      </div>
      ...
      {liveSyncStatus === LIVE_SYNC_STATUS.SCHEMA_OR_POLICY_ERROR && (
        <div className="flex items-start gap-2 px-3 py-2 rounded-xl border border-red-500/15 bg-red-500/5">
          ...
          <p className="text-2xs text-red-300 leading-relaxed">
            Sync blocked by Supabase policy. Check RLS policies for controller tables. Updates saved locally.
          </p>
        </div>
      )}
      ...
      {/* Retry */}
      {isLive && pending > 0 && !backendMissing && (
