...
// ─── RUN 11 — Additional exports for BigV backend status ──────

/**
 * isSupabaseConfigured()
 * Quick check — URL and anon key present and valid.
 * Does NOT test live connection.
 */
export function isSupabaseConfigured() {
  return isConfigValid(getSupabaseSettings())
}

/**
 * getSupabaseStatus()
 * Returns current config status without hitting network.
 * Statuses: not_configured | configured | disabled
 */
export function getSupabaseStatus() {
  const s = getSupabaseSettings()
  if (!s.enabled) return 'disabled'
  if (!isConfigValid(s)) return 'not_configured'
  return 'configured'
}

/**
 * getSchemaStatus()
 * Tests if the BigV schema exists by checking backend_health_checks table.
 * Allows schema_missing (42P01) vs connected to distinguish setup states.
 */
export async function getSchemaStatus() {
  ...
  try {
    const { error } = await client
      .from('backend_health_checks')
      .select('id')
      .limit(1)
    if (!error) return 'schema_ready'
    if (error.code === '42P01') return 'schema_missing'
    if (IGNORABLE_ERROR_CODES.has(error.code)) return 'connected'
    return 'error'
  } catch { return 'error' }
}
