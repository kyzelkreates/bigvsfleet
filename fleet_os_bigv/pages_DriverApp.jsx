...
import { DemoLiveModePanel } from './components_ui_DemoLiveToggle'
import {
...
import DriverTripPanel from './modules_ap3x_DriverTripPanel'
import { useDriverPwaStore } from './core_storage'
import { getOsmTileUrl } from './services_maps_mapProviderManager'
import {
  GPS_PERMISSION, GPS_WARNINGS, createGpsManager,
  calculateGpsConfidence, normalisePosition,
} from './services_maps_gpsManager'
import { DEMO_ROUTE, routeToLeafletPositions } from './services_maps_routeProviderManager'
    ...
    <div className="min-h-[100dvh] bg-[#060b18] flex items-center justify-center p-6">
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center space-y-2">
          ...
          <div className="text-2xl font-bold text-white tracking-tight">Big V's Best Routes™ Driver</div>
          <div className="text-sm text-slate-500">Big V's Best Routes™ — Driver App</div>
        </div>

        {/* RUN 3: Demo/Live mode indicator — role-safe, no admin controls */}
        <DemoLiveModePanel role="driver" className="mt-1" />
...
function DriverAppMain({ profile, onLogout }) {
  ...
  // ══ RUN 6: Explicit GPS permission state + Driver store ═══════
  const [gpsPermission, setGpsPermission] = useState(GPS_PERMISSION.UNKNOWN)
  const [gpsManagerRef]                   = useState(() => ({ current: null }))
  const updateGpsStatus = useDriverPwaStore(s => s.updateGpsStatus)
  const updateSyncStatus = useDriverPwaStore(s => s.updateSyncStatus)
  const startTrip6      = useDriverPwaStore(s => s.startTrip)
  const completeTrip6   = useDriverPwaStore(s => s.completeTrip)
  // Tile URL from Run 4 API config SSOT (falls back to OSM public)
  const [osmTileUrl]  = useState(() => getOsmTileUrl())
  // Demo route is shown when no GPS and no destination in demo mode
  ...
  // ══ RUN 6: Explicit GPS permission request handler ════════════
  // Driver must tap "Use My Current Location" — GPS does not auto-start separately.
  // The existing watchPosition above auto-fires on mount (existing behaviour preserved).
  // This handler updates the Run 6 gpsPermission state to match.
  const handleRequestGps6 = useCallback(() => {
    setGpsPermission(GPS_PERMISSION.REQUESTING)
    updateGpsStatus({ permission: GPS_PERMISSION.REQUESTING })
    // Sync gpsPermission from existing gpsState
    if (gpsState === 'active') {
      setGpsPermission(GPS_PERMISSION.GRANTED)
      updateGpsStatus({ permission: GPS_PERMISSION.GRANTED, confidence: 'medium' })
    } else if (gpsState === 'denied') {
      setGpsPermission(GPS_PERMISSION.DENIED)
      updateGpsStatus({ permission: GPS_PERMISSION.DENIED })
    } else {
      // Try single getCurrentPosition to trigger permission prompt
      if (!navigator.geolocation) {
        setGpsPermission(GPS_PERMISSION.UNAVAILABLE)
        updateGpsStatus({ permission: GPS_PERMISSION.UNAVAILABLE })
        return
      }
      navigator.geolocation.getCurrentPosition(
        ({ coords }) => {
          setGpsPermission(GPS_PERMISSION.GRANTED)
          updateGpsStatus({ permission: GPS_PERMISSION.GRANTED, accuracyMeters: Math.round(coords.accuracy || 0), confidence: calculateGpsConfidence(coords.accuracy) })
          if (pos == null) {
        ...
        () => {
          setGpsPermission(GPS_PERMISSION.DENIED)
          updateGpsStatus({ permission: GPS_PERMISSION.DENIED, error: 'Permission denied' })
          setGpsState('denied')
        },
        { enableHighAccuracy: true, timeout: 20000, maximumAge: 3000 }
      )
    }
  }, [gpsState, pos, updateGpsStatus])

  // ══ RUN 6: Sync gpsPermission from existing gpsState ═════════
  // Keep Run 6 permission state in sync with the existing GPS watch state.
  // Avoids duplicate GPS logic — reuses existing watchPosition.
  // Separation: existing state handles map rendering, Run 6 state handles
  // checklist/trip/compliance/store persistence.
  useEffect(() => {
    if (gpsState === 'active' && gpsPermission !== GPS_PERMISSION.GRANTED) {
      setGpsPermission(GPS_PERMISSION.GRANTED)
      updateGpsStatus({ permission: GPS_PERMISSION.GRANTED, accuracyMeters: accuracy, confidence: calculateGpsConfidence(accuracy) })
    } else if (gpsState === 'denied' && gpsPermission !== GPS_PERMISSION.DENIED) {
      setGpsPermission(GPS_PERMISSION.DENIED)
      updateGpsStatus({ permission: GPS_PERMISSION.DENIED })
    }
  }, [gpsState, accuracy])

  // ══ RUN 6: Demo route fallback — show when no GPS + no destination ═════
  useEffect(() => {
    const demoShouldShow = gpsState !== 'active' && !destination
    setShowDemoRoute(demoShouldShow)
  }, [gpsState, destination])
  ...
  const TABS = [
    ...
    { key: 'trip',   label: 'Trip',   icon: 'Navigation'     },   // RUN 6
    ...
    <div
      ...
      {tab === 'map' && (
        <div className="flex-1 relative overflow-hidden">
          ...
          {/* ── RUN 6: Advisory map banner ──────────────────────── */}
          <div className="absolute bottom-16 left-0 right-0 z-[998] pointer-events-none px-3">
            <div className="flex items-start gap-2 px-3 py-1.5 rounded-lg bg-black/70 backdrop-blur-sm border border-amber-500/15">
              ...
              <p className="text-2xs text-slate-400 leading-relaxed">
                {showDemoRoute ? 'Demo route shown. ' : ''}
                Displayed route is advisory only. GPS accuracy can vary. Obey road signs, restrictions, and road conditions.
                {gpsPermission === GPS_PERMISSION.DENIED && ' GPS denied — demo route view active.'}
              </p>
            </div>
          </div>
          ...
          {pos ? (
            mapRenderMode === 'google' && googleMapsReady ? (
              ...
              >
                <TileLayer
                  attribution='&copy; <a href="https://www.openstreetmap.org/copyright" target="_blank">OpenStreetMap contributors</a> | Map data is advisory only. Route does not guarantee legal compliance.'
                  url={osmTileUrl || 'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png'}
                />
                ...
                <div className={`max-w-[82%] flex flex-col gap-0.5 ${msg.from === 'driver' ? 'items-end' : 'items-start'}`}>
                  <div className={`flex items-center gap-1 px-0.5`}>
                    ...
                    <span className="text-2xs text-slate-600">
                      {msg.from === 'driver' ? 'You' : msg.from === 'ai' ? '4P3X AI' : 'Fleet Ops'}
                    </span>
      ...
      {tab === 'trip' && (
        <DriverTripPanel
          ...
          accuracy={accuracy}
          currentPos={pos}
          onRequestGps={handleRequestGps6}
          profile={profile}
          backendStatus={backendStatus}
          routeId={activeJob?.id || null}
          isDemo={!activeJob && showDemoRoute}
        />
      )}
