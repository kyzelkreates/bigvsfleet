...
// RUN 5: Map Provider Manager — reads Run 4 API config for tile URL
import { getOsmTileUrl, getOsmConfig, MAP_ADVISORY } from './services_maps_mapProviderManager'
import { MapModePill, MapAdvisoryBanner } from './components_ui_MapStatusPanel'
...
function resolveTileUrl(provider) {
  ...
  // RUN 5: Read tile URL from Run 4 API config SSOT (useApiConfigStore)
  // Falls back to OSM public tiles if not configured.
  // No API secrets — public tile URL only.
  const configuredUrl = getOsmTileUrl()
  if (configuredUrl && configuredUrl.includes('{z}')) return configuredUrl
...
const ApexMap = forwardRef(function ApexMap({
  ...
  height        = '100%',
  className     = '',
  // RUN 5: Map Engine Foundation props
  isDemo        = false,       // true when demo route/data is shown
  showAdvisory  = false,       // show advisory banner overlay on map
  activeMode    = 'osm_2d',    // from mapProviderManager
  mapStatusCompact = false,    // show compact mode pill overlay
}, ref) {
  ...
  return (
    <div className={`relative overflow-hidden ${className}`} style={{ height }}>
      ...
      {/* RUN 5: Map advisory banner overlay */}
      {showAdvisory && (
        <MapAdvisoryBanner
          isDemo={isDemo}
      ...
      {mapStatusCompact && (
        <MapModePill
          ...
          isDemo={isDemo}
          className="absolute top-2 left-2 z-[500] pointer-events-none"
        />
      )}
